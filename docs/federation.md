...
the federated metrics contain a mix of different sample types (floats of any flavor, counter
histograms, or gauge histograms) for the same metric name, the federation payload
will contain multiple metric families with the same name (but different types – for technical reasons, float samples are always federated as `untyped`, while histogram samples are federated with their full type information).
